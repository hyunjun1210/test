...
function render() {
  ...
  if (isAdmin) {
    ...
    document.getElementById('save-teams-btn').addEventListener('click', () => {
      ...
      const updatedTeams = newNames.map((name, index) => {
        const existingTeam = teamsData[index];
        return {
          id: existingTeam?.id || `team${Date.now() + index}`,
          name: name,
          totalInvestment: existingTeam?.totalInvestment || 0,
        };
      });
    ...
    if (!currentStudent) {
      ...
      app.innerHTML = `
        <section id="student-login">
          ...
          <div class="input-group">
            <label for="student-id">학번 (예: 10315)</label>
            <input type="text" id="student-id" placeholder="학년/반/번호 (5자리)" required>
          </div>
          <div class="input-group">
      ...
      document.getElementById('start-investment-btn').addEventListener('click', () => {
        ...
        if (studentId && studentName) {
          ...
          // Firebase에 학생 정보 저장 또는 업데이트
          firebase.database().ref(`investors/${studentId}`).transaction((currentData) => {
            if (currentData === null) {
              return { studentId, name: studentName, totalInvestment: 0, investments: {} };
            }
            return currentData; // 이미 있으면 그대로 둠
          });
          render(); // 투자 화면으로 전환
        } else {
      ...
      app.innerHTML = `
        <section id="student-mode">
          ...
          <div id="investment-status">
            <p>총 투자 가능 금액: 10,000만원</p>
            <p>현재 투자액: <span id="current-investment">0</span>만원</p>
            <p>남은 투자 가능액: <span id="remaining-investment">10,000</span>만원</p>
          </div>
          <div class="button-group">
      ...
      const updateInvestmentStatus = () => {
        const investedAmount = Object.values(currentStudent.investments).reduce((sum, inv) => sum + inv.amount, 0);
        document.getElementById('current-investment').textContent = investedAmount.toLocaleString();
        document.getElementById('remaining-investment').textContent = (10000 - investedAmount).toLocaleString();
      };
      ...
      teamsData.forEach(team => {
        ...
        li.innerHTML = `
          ...
          <div class="invest-form">
            <input type="number" class="investment-amount" placeholder="투자 금액 (만원)" min="1" max="10000">
      ...
      document.querySelectorAll('.invest-btn').forEach(button => {
        button.addEventListener('click', (e) => {
          ...
          const investedAmount = Object.values(currentStudent.investments).reduce((sum, inv) => sum + inv.amount, 0);
          const currentTeamInvestment = currentStudent.investments[teamId]?.amount || 0;
          
          if (investedAmount - currentTeamInvestment + amount > 10000) {
            alert('총 투자 금액 1,000만원을 초과할 수 없습니다.');
            return;
          }

          // ✅ Firebase에 투자 정보 업데이트
          const updates = {};
          updates[`investors/${currentStudent.studentId}/investments/${teamId}`] = { teamId, amount };
          updates[`teams/${teamsData.findIndex(t => t.id === teamId)}/totalInvestment`] = (teamsData.find(t => t.id === teamId).totalInvestment || 0) - currentTeamInvestment + amount;
          firebase.database().ref().update(updates)
            .then(() => {
              currentStudent.investments[teamId] = { teamId, amount };
              updateMyInvestments();
      ...
      const updateMyInvestments = () => {
        ...
        Object.values(currentStudent.investments).forEach(inv => {
          ...
          if (team) {
            const li = document.createElement('li');
            li.innerHTML = `<strong>${team.name}</strong>: ${inv.amount}만원`;
      ...
      document.getElementById('clear-investments-btn').addEventListener('click', () => {
          ...
          const updates = {};
          Object.values(currentStudent.investments).forEach(inv => {
              const teamIndex = teamsData.findIndex(t => t.id === inv.teamId);
              if (teamIndex !== -1) {
                  const currentTeamInvestment = teamsData[teamIndex].totalInvestment || 0;
                  updates[`teams/${teamIndex}/totalInvestment`] = currentTeamInvestment - inv.amount;
              }
          });
...
teamRef.on('value', (snapshot) => {
  ...
  teamsData = data ? Object.values(data) : [];
  render();
});
...
function handleFileUpload(e) {
  ...
  reader.onload = (evt) => {
    ...
    const newTeams = names.map((name, index) => {
      const existingTeam = teamsData[index];
      return {
        id: existingTeam?.id || `team${Date.now() + index}`,
        name: name,
    ...
    firebase.database().ref('teams').set(newTeams)
