    ...
    if (isAdmin) { 
        ...
        if (!currentStudent) { 
            ...
            app.innerHTML = ` 
                <section id="student-mode"> 
                    ...
                    <div id="investment-status"> 
                        <p>총 투자 가능 금액: 10,000,000원</p> 
                        <p>현재 투자액: <span id="current-investment">0원</span></p> 
                        <p>남은 투자 가능액: <span id="remaining-investment">10,000,000원</span></p> 
                    </div> 
                    <hr> 
                    <h3>창업팀 목록</h3> 
                    <ul id="team-list"></ul> 
            ...
            const updateInvestmentStatus = () => { 
                ...
                document.getElementById('current-investment').textContent = investedAmount.toLocaleString() + '원'; 
                document.getElementById('remaining-investment').textContent = (10000000 - investedAmount).toLocaleString() + '원'; 
            }; 
        ...
        firebase.database().ref().update(updates)
            ...
            const updateMyInvestments = () => { 
                ...
                Object.values(currentStudent.investments).forEach(inv => { 
                    ...
                    if (team) { 
                        const li = document.createElement('li'); 
                        li.innerHTML = `<strong>${team.name}</strong>: ${inv.amount.toLocaleString()}원`; 
